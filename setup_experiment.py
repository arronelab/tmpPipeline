...
if not fp_file_exists:
    print('Missing fingerPrint1.dat, run fingerPrint.py first')
coord_file_exists = exists(sys.argv[1]+'coordinates1.dat')
if not coord_file_exists:
    print('Missing coordinates1.dat, run coordinates.py first')
...
how_many_preds = input('How many predictions would you like to run? (Default = 40)\n')
...
def write_sh_file(test_name,project_name):
    ...
    with open(output_filepath, 'w+') as fout:
        ...
        fout.write('\ninitialCoordsFile={}coordinates1.dat'.format(test_name))
        if is_mix=='Y':
            fout.write('\nnoStructures=1')
        else:
        ...
        if fix_all == 'N':
            fout.write('\nfixedsections={}varyingSectionSecondary1.dat'.format(test_name))
        else:
            fout.write('\nfixedsections=none')
